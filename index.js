		...
		try {
			...
			switch(command) {
				...
				case 'gtts':
					...
					if (args.length < 2) return client.sendMessage(from, 'TEXTO NE AMIGO KKKK', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('lTEXTO MT GRANDE')
				...
				case 'nsfwloli':
					...
					break
				case 'hilih':
					if (args.length < 1) return reply('SLA MN ;-;')
				...
				case 'ytsearch':
					...
					break
				case 'tiktok':
					if (args.length < 1) return reply('CD A URL MEM?')
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'QUAL O USERNAME DO BRODI?', text, {quoted: mek})
				...
				case 'tstiker':
				case 'tsticker':
					if (args.length < 1) return reply('vai escrever oq mem?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'clearall':
					...
					reply('Sukses apagar todo o chat:)')
					break
				case 'bc':
					if (!isOwner) return reply('Quem é Você?')
				...
				case 'add':
					...
					if (args.length < 1) return reply('VAI ADD O FANTASMA KR***?')
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						teks = 'BAN DO ADM ON FICA ESPERTO PRA VC N RODARKKKKK :\n'
